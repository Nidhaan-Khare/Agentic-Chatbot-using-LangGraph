...
#llm=ChatOpenAI()
#llm = ChatGoogleGenerativeAI(model="gemini-2.0-flash" , transport="rest" , thinking_budget=0)
llm = ChatGroq(
    model="llama-3.3-70b-versatile",
    temperature=0
)
#print(llm.invoke("PM of india is?"))
...
def get_stock_price(symbol: str) -> dict:
    ...
    """
    url = f"https://www.alphavantage.co/query?function=GLOBAL_QUOTE&symbol={symbol}&apikey=V8MOUD747FNER0QF"
    r = requests.get(url)
    return r.json()
...
def purchase_stock(symbol: str, quantity: int) -> dict:
    ...
    if isinstance(decision, str) and decision.lower() == "yes":
        ...
        return {
            "status": "cancelled",
            "message": f"Purchase of {quantity} shares of {symbol} was declined by you.",
            "symbol": symbol,
            "quantity": quantity,
        }
...
#     {
#         "messages": [
#             HumanMessage(content="Hello, who is India's PM ?")
#         ]
#     },
#     config=CONFIG
# )
